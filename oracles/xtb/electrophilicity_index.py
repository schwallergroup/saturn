...
class ElectrophilicityIndex(OracleComponent):
    ...
    def __call__(self, mols: np.ndarray[Mol]) -> np.ndarray[float]:
        raw_electrophilicity_index_values = []
        for mol in mols:
            try:
                ...
                raw_electrophilicity_index_values.append(E_index)

            except Exception:
                # FIXME: could be dangerous as 0.0 may actually be a good value
                raw_electrophilicity_index_values.append(0.0)

        return np.array(raw_electrophilicity_index_values, dtype=np.float32)
