    ...
    def __call__(self, mols: np.ndarray[Mol]) -> np.ndarray[float]:
        ...
        for mol in mols:
            try:
                ...
                raw_nucleophilicity_values.append(xtb.get_global_descriptor("nucleophilicity", corrected=True))

            except Exception:
                # FIXME: could be dangerous as 0.0 may actually be a good value
                raw_nucleophilicity_values.append(0.0)

        return np.array(raw_nucleophilicity_values, dtype=np.float32)
