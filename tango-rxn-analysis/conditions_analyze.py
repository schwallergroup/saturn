...
def log_pooled_molecules_metrics(
    pooled_data: List[Tuple[str, float, float, Optional[float]]],  # (SMILES, docking_score, qed, hbd)
    threshold_label: str,
    ...
    if pooled_data:
        ...
        if include_hbd:
            ...
            logging.info(f"{threshold_label}: Docking Scores: {np.mean(docking_scores):.2f} ± {np.std(docking_scores):.2f}, QED: {np.mean(qed_values):.2f} ± {np.std(qed_values):.2f}")
...
def log_molecule_and_rxn_metrics(
    ...
    for seed_path in seeds_paths:
        ...
        # add QED if not in columns 
        if QED_ENUM not in df_enforced_rxn.columns:
            def compute_qed_from_smiles(smiles: str) -> float:
                mol = Chem.MolFromSmiles(smiles)
                return QED.qed(mol) if mol is not None else np.nan

            df_enforced_rxn[QED_ENUM] = df_enforced_rxn["canonical_smiles"].apply(compute_qed_from_smiles)
        ...
        hbd_values = df_enforced_rxn[HBD_ENUM] if include_hbd else [None] * len(df_enforced_rxn)
        ...
        hbd_values_top = df_top_enforced_rxn[HBD_ENUM] if include_hbd else [None] * len(df_top_enforced_rxn)
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--log_rxn_metrics",
        action="store_true",
        help="Whether to log reaction-level metrics and plots."
    )
    parser.add_argument(
        "--compute_hbd",
        action="store_true",
        help="Whether to compute and log H-bond donor counts (requires the HBD column in the oracle history)."
    )
    ...
    for experiment_name in args.experiments:
        ...
        if args.log_rxn_metrics:
            log_molecule_and_rxn_metrics(
                ...
                include_hbd=args.compute_hbd
            )
        else:
            logging.info("Skipping reaction metrics logging (enable with --log_rxn_metrics).")
