...
class LargeBeamEnumeration:
    ...
    def exhaustive_beam_expansion(self, agent: Generator) -> List[str]:
        ...
        for time_step in range(1, self.beam_steps + 1, 1):
            ...
            if self.token_sampling_method == 'topk':
                curr_step_probs, top_indices = torch.topk(probabilities, self.k)
            ...
            if time_step == 1:
                ...
                enumerated_sequence_probs = curr_step_probs[0]
            ...
            # otherwise, each sub-sequence needs to be extended by its top k (or sampled) tokens
                ...
                for sub_sequence, top_tokens, sub_sequence_prob, step_probs in zip(enumerated_sequences, 
                                                                                   top_indices,
                                                                                   enumerated_sequence_probs,
                                                                                   curr_step_probs):
                    for token, prob in zip(top_tokens, step_probs):
                        temp.append(torch.cat([sub_sequence, token.unsqueeze(0)]))
                        temp_probs.append(float(sub_sequence_prob) * float(prob))


                # at this point, len(temp_sequences) > len(enumerated_sequences) because of the beam expansion
                enumerated_sequences = temp
                enumerated_sequence_probs = temp_probs
        ...
        # keep 2^beam_fraction beams
        beam_fraction = 6
        enumerated_sequences, enumerated_sequence_probs, input_vector, hidden_state = self.discard_sequences(beam_fraction=beam_fraction,
                                                                                                             enumerated_sequences=enumerated_sequences,
                                                                                                             enumerated_sequence_probs=enumerated_sequence_probs,
                                                                                                             input_vector=input_vector,
                                                                                                             hidden_state=hidden_state)
        
        # ------------------------------------------------------------------------------------------
        # FIXME: temporary expansion phase 2
        for time_step in range(1, 18 - beam_fraction - 1, 1):
            logits, hidden_state = agent.network(input_vector.unsqueeze(1), hidden_state)
            logits = logits.squeeze(1)
            probabilities = logits.softmax(dim=1)
            ...
            # initialize a temporary list that stores all the sub-sequences from this time-step
            temp = []
            # FIXME: don't care about probability in phase 2
            #temp_probs = []
            # for each sub-sequence, extend it by its most probable k tokens (or sampled)
    ...
    def discard_sequences(
        ...
        # Enumerate the list and sort by value in descending order
        indexed_list = sorted(enumerate(enumerated_sequence_probs), key=lambda x: x[1], reverse=True)

        # Extract the top k indices
        keep_indices = [idx for idx, _ in indexed_list[:int(math.pow(self.k, beam_fraction))]]

        # slice 
        enumerated_sequences = [enumerated_sequences[idx] for idx in keep_indices]
        enumerated_sequence_probs = [enumerated_sequence_probs[idx] for idx in keep_indices]
        input_vector = input_vector[keep_indices]
        hidden_state = (hidden_state[0][:, keep_indices, :], hidden_state[1][:, keep_indices, :])
        
        
        return enumerated_sequences, enumerated_sequence_probs, input_vector, hidden_state
